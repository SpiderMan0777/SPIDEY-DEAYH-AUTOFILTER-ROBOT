# Don't Remove Credit @Spidey_Botz
# Subscribe YouTube Channel For Amazing Bot @_Spidey
# Ask Doubt on telegram @KingSpidey01
...
from database.ia_filterdb import col, sec_col, db as Spideydb, sec_db, get_file_details, get_search_results, get_bad_files
...
async def pm_text(bot, message):
    ...
    # Ignore if it's a command or hashtag
...
async def next_page(bot, query):
    ...
    # Duplicate remove: Unique file_id only
    unique_files = []
    file_ids = set()
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith(('[' , '@', 'www.')), file['file_name'].split()))}",
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Spidey_bots")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://github.com/SpideyBots/Spidey-FILTER-BOT")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingSpidey01")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, Spidey_search):
    ...
    if AI_SPELL_CHECK == True and Spidey_search == True:
        ...
        Spidey_ai_msg = await reply_msg.edit_text("<b><i>I Am Trying To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
